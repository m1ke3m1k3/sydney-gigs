...
async function loadData() {
  try {
    ...
    document.getElementById('cards-grid').innerHTML =
      '<p style="color:var(--text-muted); grid-column:1/-1;">Error cargando oportunidades. Comprueba que el scraper haya generado el archivo JSON.</p>';
  }
}
...
function renderCards() {
  ...
  const filtered = allOpportunities.filter(opp => {
    ...
    const locationMatch =
      ...
      (activeFilters.location === 'online' && locLower.includes('online')) ||
      (activeFilters.location === 'presencial' && !locLower.includes('online'));
