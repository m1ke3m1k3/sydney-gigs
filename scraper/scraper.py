    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for card in cards[:30]:
                ...
                title = title_el.get_text(strip=True) if title_el else "Proyecto sin título"
                ...
                pay = pay_el.get_text(strip=True) if pay_el else "Ver sitio"
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for card in cards[:30]:
                ...
                pay_match = re.search(r"\$\s*([\d,]+)", text)
                pay = f"${pay_match.group(1)}" if pay_match else "Ver sitio (compensación variable)"
                ...
                results.append({
                    ...
                    "pay": pay,
                    "type": "Clinical Study",
                    "location": "Sydney (presencial, supervisión médica)",
                    "link": link,
                    "scraped_at": datetime.now().isoformat()
                })
        except Exception as e:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for card in cards[:30]:
                ...
                pay_match = re.search(r"\$\s*([\d,]+)", text)
                pay = f"${pay_match.group(1)}" if pay_match else "Reembolso de tiempo/gastos"
